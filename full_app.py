...
# Function to save and process the uploaded image (for Manual Mode)
def save_and_process_image_auto(image: Image.Image, user_question: str):
    # Save the uploaded image
    image_path = "uploaded_image.png"
    image.save(image_path)
    st.write(f"Image saved at: {image_path}")

    # Pass the saved image to the LVLM function (Manual Mode)
    result = lvlm(image, user_question)
    return result

# Function to communicate with the LVLM (OpenAI)
...
# Function to save and process the uploaded image (for Auto Mode with Canvas)
def save_and_process_image_manual(image: Image.Image, user_question: str):
    # Save the uploaded image
    image_path = "uploaded_image.png"
    image.save(image_path)
    st.write(f"Image saved at: {image_path}")
...
def main():
    ...
    # Select Mode: Auto or Manual
    mode = st.radio("Select Mode", ("Manual Mode", "Auto Mode"))

    if mode == "Manual Mode":
        st.subheader("Manual Mode: Image Upload, Crop, and LVLM Integration")
                ...
                if len(shapes) > 0:
                    ...
                    if st.button("Analyze Cropped Image"):
                        with st.spinner("Processing..."):
                            result = save_and_process_image_manual(cropped_img, user_question)
                            st.write(result)
        ...
        st.subheader("Auto Mode: Enter Custom Input")

        # User can input any text for the assistant to process
        user_question = st.text_input("Enter your question:")
        ...
        if uploaded_file is not None:
            ...
            # Now use the image for LVLM processing
            if st.button("Analyze Uploaded Image"):
                with st.spinner("Processing..."):
                    result = save_and_process_image_auto(img, user_question)
                    st.write(result)
